...
function PoissonDiskGenerator( numPoints, minDistance ) {
...
PoissonDiskGenerator.prototype = {
  ...
  generatePoints: function() {
    ...
    do {
      ...
      inCircle = isInCircle(FirstPoint.x, FirstPoint.y);
  	} while (!inCircle);
    ...
    // generate new points for each point in the queue
  	while ( (ProcessList.length !== 0) && (SamplePoints.length < this.numPoints) )
  	{
        ...
        var fraction = 1;//minVal * ( 1.0 - t) + maxVal * t;
  			var NewPoint = this.generateRandomPointAround( Point, fraction * this.minDistance );

        var inCircle = isInCircle(NewPoint.x, NewPoint.y);
